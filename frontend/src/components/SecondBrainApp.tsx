...
import { 
  ...
  Search, File, Image, Filter, RotateCcw ,ExternalLink
} from 'lucide-react';
...
const SecondBrainApp: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        // Fetch file data for PDFs and images
        const enhancedData = await Promise.all(dataArray.map(async (item: any) => {
          const mappedItem = {
            ...item,
            id: item._id || item.id,
          ...
          // For PDFs and images, fetch the file data separately if needed
          if ((item.type === 'pdfs' || item.type === 'images') && !item.fileData) {
            try {
              const fileResponse = await axios.get(`${API_URL}${item._id}/preview`, {
                ...config,
                responseType: 'blob'
              });
              ...
              mappedItem.fileData = base64;
            } catch (fileError) {
              console.error(`Failed to fetch file data for ${item._id}:`, fileError);
            }
          }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.videoCount) formDataToSend.append('videoCount', formData.videoCount.toString());
      ...
      if (!newContent._id && response.data.data) {
      ...
      // For files, ensure fileData is properly formatted for display
      if (selectedFile && (selectedType === 'pdfs' || selectedType === 'images')) {
        if (!processedContent.fileData) {
          // Create data URL from the selected file for immediate display
          const reader = new FileReader();
          reader.onload = (e) => {
            processedContent.fileData = e.target?.result as string;
            setContents(prev => [processedContent, ...prev]);
          };
          reader.readAsDataURL(selectedFile);
        } else {
          // If fileData is base64, convert to data URL
          if (!processedContent.fileData.startsWith('data:')) {
            processedContent.fileData = `data:${processedContent.fileType};base64,${processedContent.fileData}`;
          }
          setContents(prev => [processedContent, ...prev]);
        }
      } else {
            ...
            <div className="relative">
              {content.fileData ? (
                <img 
                  ...
                  onError={(e) => {
                    console.error('Image load error for:', content.fileName);
...
return (
  <div className="h-screen bg-gray-900 text-white flex overflow-hidden">
    ...
    {/* Main Content */}
    <main className={`flex-1 flex flex-col transition-all duration-300 ${previewContent ? 'mr-0' : ''} overflow-hidden`}>
      {/* Fixed Header */}
      <header className="bg-gray-900 border-b border-gray-700 p-6 flex-shrink-0">
